...
# Ground truth labels from Step 2 (manual benchmarking)
GROUND_TRUTH = {
...
def get_agent_recommendation(kernel_name, source_code, timings):
    ...
    messages = [
        SystemMessage(content=(
            "You are a CUDA GPU performance expert. "
        ...
        HumanMessage(content=(
            ...
            f"What is your final memory type recommendation?"
        ))
    ]
...
def extract_recommendation(agent_answer):
    """Extract clean recommendation from agent response."""
    # Try to find explicit RECOMMENDATION: line
    match = re.search(r'RECOMMENDATION:\s*(DEVICE|HOST|UNIFIED)', agent_answer, re.IGNORECASE)
    if match:
        return match.group(1).lower()
    # Fallback: look for memory type mentions
